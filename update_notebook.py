...
md_cell = {
    ...
    'source': [
        ...
        '\n',
        'In addition to our fully deployed Streamlit dashboard (`ui/app.py`), we encapsulate the entire end-to-end workflow into a single prediction pipeline function below. This pipeline takes raw customer data, preprocesses it, filters the features using our GA results, assigns a K-Medoids cluster, and computes a Fuzzy Action Score.'
    ]
}
...
# Find where to insert (before conclusion or at the end)
insert_idx = len(nb['cells'])
for i, cell in enumerate(nb['cells']):
    if cell['cell_type'] == 'markdown':
        if len(cell['source']) > 0 and '# 8. Conclusion' in cell['source'][0]:
            insert_idx = i
            break
...
for cell in nb['cells']:
    if cell['cell_type'] == 'markdown' and len(cell['source']) > 0 and '7. System Implementation' in cell['source'][0]:
        already_exists = True

if not already_exists:
    nb['cells'].insert(insert_idx, md_cell)
    nb['cells'].insert(insert_idx + 1, code_cell_1)
    nb['cells'].insert(insert_idx + 2, code_cell_2)

with open(notebook_path, 'w', encoding='utf-8') as f:
    json.dump(nb, f, indent=1)

print('Successfully added pipeline to notebook.')
